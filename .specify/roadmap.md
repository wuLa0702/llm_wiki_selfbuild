...
Phase 3          ████████████████████  100% ✅
Phase 4          ████████████████████  100% ✅
Phase 5（当前）    ░░░░░░░░░░░░░░░░░░░░   0%  搜索增强 + 文档格式 + 向量搜索
Phase 6          ░░░░░░░░░░░░░░░░░░░░   0%  UI 美化
Phase 7          ░░░░░░░░░░░░░░░░░░░░   0%  多模态 + 企业级
Phase 8          ░░░░░░░░░░░░░░░░░░░░   0%  打包发布（解压即用）

面试就绪线 ──────────────────────────── Phase 6 完成即可达到
```
...
## Phase 5：搜索增强 + 文档格式 + 向量搜索 🔜

> **状态**：🟡 方案设计中
> **对标**：nashsu v0.4 搜索体系
> **前置**：Phase 4 完成后（知识图谱 + MCP + 队列稳定）

### 已完成的 Phase 4.5 基础 ✅

以下功能已在 Phase 4.5 实现，是 Phase 5 搜索能力的基础：

| 功能 | 说明 |
|------|------|
| **来源可追溯** | ingest 时记录源文件 → wiki 页面可追溯回 raw source |
| **Embedding 索引** | 基础向量化索引，为语义搜索铺垫 |
| **语言感知生成** | output_language 注入所有 LLM prompt |
| **队列可视化** | `/wiki/queue` 活动面板 |

### Phase 5 新增功能

| 功能 | 说明 | 对标 |
|------|------|:----:|
| **PPTX/XLSX 文档解析** | 支持 `python-pptx` 解析 PPTX、`openpyxl` 解析 XLSX，提取文本内容结构化后 ingest | 🆕 |
| **CJK Bigram 分词 + 停用词** | 当前 SQLite `LIKE` 不支持中文分词。改用 CJK Bigram（中文二元分词）+ 停用词过滤，提升中文搜索准确率 | ✅ nashsu |
| **BM25 关键词搜索** | 基于 BM25 算法的全文检索，标题/正文加权，比 SQLite LIKE 精确一个数量级 | ✅ nashsu |
| **向量语义搜索** | embedding + LanceDB，任意 OpenAI 兼容端点，解决"语义相近但关键词不同"的问题 | ✅ nashsu |
| **混合搜索（RRF 融合）** | BM25 + 向量语义 RRF 融合排序，recall 提升 ~13% | ✅ nashsu |
| **i18n 国际化** | 页面内容/UI 支持中英文切换，配置文件驱动 | 🆕 |

### 面试可讲

- "我支持 PPTX/XLSX 解析——这不仅仅是加个库，而是针对办公场景的工程落地，面试可以体现产品思维"
- "搜索从 Phase 3 的 SQLite LIKE 升级为 BM25 + 向量语义 RRF 融合，recall 提升超过 10%"
- "CJK Bigram 分词是中文搜索的基本功——单纯用 LIKE 做中文搜索，'机器学习'搜不到'机器 学习'"
- "i18n 用配置文件驱动，不硬编码——新增语言只需加一个 YAML 文件"
...
1. **不碰后端逻辑**——Phase 6 只做前端/静态层美化，不改 API、不重构核心
...
| **Source 自动监听（完整版）** | 检测 raw/sources/ 删除 → 清理孤儿 wiki；检测修改 → 重新 ingest | Phase 3 基础版 → 完整版 |
| **图谱洞察 → 一键 Deep Research** | 从知识空白卡片直接触发网络搜索 | Phase 4 洞察 + 网络搜索联调 |

---

## Phase 8：打包发布（PyInstaller + 解压即用）

> **状态**：⬜ 未开始
> **对标**：桌面应用的安装体验
> **前置**：Phase 7 完成后（或 Phase 5 完成后提前启动）

### 目标

用户下载 zip → 解压 → 双击 exe → 浏览器打开即用。无需安装 Python、无需命令行。

### 方案

```
LLM-Wiki-v1.0-win64.zip
├── LLM-Wiki.exe              ← PyInstaller --onefile 打包
├── .models/
│   └── all-MiniLM-L6-v2/     ← 预下载的 embedding 模型
├── static/                   ← 前端资源
├── README.txt
└── start.bat                 ← 可选
```
...
- [ ] **路径规范化** — `src/config.py` 检测是否在 PyInstaller 环境，计算 exe 同级目录为工作根目录
- [ ] **入口脚本** — `src/entry.py` PyInstaller 启动入口：启动 Uvicorn + 自动打开浏览器
- [ ] **资源打包** — `src/api/templates/` 和 `static/` 通过 `--add-data` 打包进 exe
- [ ] **模型分发** — `.models/` 随 zip 发放，exe 检测不存在时给友好提示
- [ ] **PyInstaller 配置** — `build.spec` 文件：隐藏控制台、指定图标、排错配置
- [ ] **一键打包脚本** — `scripts/build-release.sh` / `.bat`：`pip install pyinstaller && pyinstaller build.spec && zip -r dist/`
- [ ] **UI 适配** — PyInstaller 环境下的 HTML 模板路径和 `static/` 路径处理
- [ ] **README + 使用说明** — 解压即用流程、首次启动指南
- [ ] **GitHub Release** — 自动上传 zip 到 Releases

### 面试可讲

- "我用 PyInstaller 把项目打包成单个 exe，用户解压即用"
- "PyInstaller 环境下路径处理是坑——我通过 `sys.executable` 判断运行环境来区分开发和生产路径"
- "Embedding 模型预下载好随包发放，用户不需要额外下载"
...
6. **搜索"机器学习" → BM25 + 向量语义返回准确结果，中英文都支持**
7. **导入一个 PPTX 文件 → 自动提取文本生成 wiki 页面**
8. Claude Code 通过 MCP 直接搜索我的 wiki → 实时演示
9. **打开 `/wiki` → 暗色模式、移动端适配、搜索框、骨架屏——不像 demo，像产品**
...
| PPTX/XLSX 解析 | 中 | 5 | 办公文档支持，体现产品思维 |
| BM25 + 向量混合搜索 | 高 | 5 | LanceDB + BM25 RRF 融合 |
| CJK Bigram 中文分词 | 中 | 5 | 中文搜索基本功 |
| i18n 国际化 | 低 | 5 | 配置驱动，工程落地 |
| **UI 细节打磨** | 低 | 6 | 骨架屏、暗色模式、移动端适配、hover 预览 |
| 多模态图片摄入 | 高 | 7 | 视觉模型集成 |
...
| Phase 3 | 2-3 周（已完成 ✅） | Query + 图解析 + 静态 Lint + Source 监听 |
| Phase 4 | 3-5 周（已完成 ✅） | 四信号图谱 + Louvain + MCP + 批量导入 |
| Phase 5 | 4-6 周 | 搜索增强 + 文档格式 + 向量搜索 + i18n |
| Phase 6 | 2-3 周 | UI 美化 |
| Phase 7 | 按需 | 多模态 + 企业级 |
| Phase 8 | ~1 周 | PyInstaller 打包 + 路径适配 + 自动发布 |
| **面试就绪** | **Phase 6 完成** | 核心差异化 + 搜索 + 可展示的 UI |
| **合计（到 Phase 6）** | **约 16-24 周** | 完整功能闭环 + UI 可展示 |
