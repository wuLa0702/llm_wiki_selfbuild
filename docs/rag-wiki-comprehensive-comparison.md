...
| # | 问题 | 一句话回答 | 优先级 |
|---|------|-----------|--------|
| 1 | **项目有没有 RAG？RAG 和 Wiki 定位有何不同？** | 项目是 **Wiki 知识库**（结构化知识沉淀），**没有传统 RAG 的 chunk-召回-生成 流水线**。首版对标 Karpathy 的 LLM Wiki 理念——"编译"而非"检索"。 | 🔴 P0 |
| 2 | **向量召回、关键词召回有没有？怎么实现的？** | **两者都有**。BM25 关键词搜索 + Chroma 向量语义搜索 + **RRF 融合**（hybrid 模式），启闭由配置开关控制。 | 🔴 P0 |
| 3 | **有没有做 chunk 切分？向量化怎么做的？** | **没有做 chunk 切分**。整页编码（前 8000 字符），这可能是一个需要改进的短板。 | 🔴 P0 |
| 4 | **向量维度是多少？是稠密还是稀疏？** | **384 维稠密向量**（all-MiniLM-L6-v2），不是 128 维。使用 cosine 距离。 | 🟠 P1 |
| 5 | **Query 模糊/歧义如何处理的？** | **没有显式的模糊处理**。依赖 ReAct Agent 的"搜索→阅读→推理"循环，以及 LangGraph 的三层自修正机制。 | 🟠 P1 |
| 6 | **Embedding 模型有做 A/B 测试吗？评估指标如何设计？** | **当前没有**。首版卡在"有比没有好"阶段，评估体系留待 Phase 5。 | 🟠 P1 |
| 7 | **单 Agent 项目需要上下文压缩和记忆存储吗？做到了哪一步？** | **已经做了，而且做得比较多**——MemorySaver + SQLite 持久化 + Attention Sink + Working Memory + 摘要压缩 + 归档降级。 | 🟠 P1 |
| 8 | **项目有哪些特色亮点可以写简历？** | ReAct + 三层自修正、BM25+向量+RRF 融合搜索、Attention Sink、Human-in-the-Loop 审批、SSE 流式、记忆降级归档。 | 🟡 P2 |
| 9 | **前端能用吗？项目能部署吗？** | **能**——React 前端 + FastAPI 后端 + Vite 代理。部署缺 Dockerfile，需手动部署。 | 🟡 P2 |
| 10 | **MCP Server 怎么接入的？需要升级吗？** | 已实现 8 个 MCP 工具（`@wiki_tool` 装饰器），但 Agent 内部工具与 MCP 工具**两套重复定义**，建议统一为 MCP-first 架构。 | 🟠 P1 |
| 11 | **上下文管理对标 LangGraph 官方体系如何？** | 自研记忆层功能完整（MemorySaver + SQLite + Attention Sink + WM），但**缺 SqliteSaver、BaseStore、Time Travel、Delta Channels**。P1 最少需升级 SqliteSaver。 | 🔴 P0 |
| 12 | **未来多 Agent 方向怎么走？** | 2026 官方推荐 **Subagents as Tools**（主管 Agent 通过 @tool 调子 Agent），当前项目基础设施大部分可直接复用。 | 🟠 P1 |
| 13 | **LangSmith 集成和前端验证怎么样？** | 当前**零集成**（无 LANGCHAIN_TRACING_V2），加 3 行 .env 配置即可启用。前端 SSE 流已通但**缺审批 UI、会话列表同步、来源引用渲染**。 | 🟠 P1 |
