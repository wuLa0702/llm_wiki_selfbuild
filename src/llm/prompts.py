...
所有页面间引用的 `[[wikilinks]]` 必须使用完整相对路径，包含目录前缀和 `.md` 扩展名。禁止使用裸名。

格式: `[[目录/文件名.md|显示名]]` 或 `[[目录/文件名.md]]`

正确示例:
- `[[entities/毕木.md]]`          — 引用实体页面
- `[[concepts/社交减法.md]]`       — 引用概念页面
- `[[sources/掌控习惯.md]]`        — 引用来源页面
- `[[concepts/专注.md|深度专注]]`   — 带自定义显示名
