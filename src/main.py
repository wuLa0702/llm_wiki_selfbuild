...
async def ingest_folder(request: Request):
    ...
    body:
        ...
        mode: "sync"（同步，等待结果）或 "async"（异步，加入队列）
    """
    ...
    if mode == "async":
        return importer.import_folder_async(folder_path, _ingest_queue, recurse=recurse)
    else:
        compiler = WikiCompiler(task_queue=_task_queue)
...
@app.get("/wiki/import", response_class=HTMLResponse)
